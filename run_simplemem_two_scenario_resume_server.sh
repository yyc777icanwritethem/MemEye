...
mkdir -p runs
exec > runs/simplemem_two_scenario_resume_v1.log 2>&1
source /data/yangyicheng/conda/miniconda3/etc/profile.d/conda.sh
conda activate mmdm
set -a
source /home/yangyicheng/.config/mmdm/writer_qwen.env
set +a
# GPU2 is currently occupied by another user's process. Omni's local work is
# MiniLM embedding only; keep this recovery isolated on CPU.
export CUDA_VISIBLE_DEVICES=""
export HF_HOME=/data/yangyicheng/huggingface_cache

echo "stage=home_resume status=started"
python -u run_benchmark.py \
  ...
  --mode open
echo "stage=home_resume status=completed"
