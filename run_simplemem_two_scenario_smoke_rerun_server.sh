...
mkdir -p runs
exec > runs/simplemem_two_scenario_smoke_promptfix_v2.log 2>&1
...
echo "stage=home_reuse status=started"
python -u run_benchmark.py \
  ...
  --output-root runs/omni_two_scenario_smoke_promptfix_v2 \
  --mode open
echo "stage=home_reuse status=completed"

echo "stage=health_reuse status=started"
python -u run_benchmark.py \
  ...
  --output-root runs/omni_two_scenario_smoke_promptfix_v2 \
  --mode open
echo "stage=health_reuse status=completed"
